...
def signin(request):
    if not request.method == 'POST':
        return JsonResponse({'error':'Send post with correct parameter'})
...
#validation part
    ...
    if len(password)<3:
        return JsonResponse({'error':'Password legth'})
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_permission(self):
        try:
            ...
            return [permission() for permission in permission_classes_by_action]